export declare type FindOrFilter = 'isFind' | 'isFilter';
export declare type QueryStatus = 'notQueried' | 'queried';
export interface DeepReadonlyArray<T> extends ReadonlyArray<DeepReadonly<T>> {
}
export declare type DeepReadonlyObject<T> = {
    readonly [P in keyof T]: DeepReadonly<T[P]>;
};
export declare type DeepReadonly<T> = T extends (infer R)[] ? DeepReadonlyArray<R> : T extends object ? DeepReadonlyObject<T> : T;
export declare type UpsertableObject<T, S> = WithOne<T> & WithMany<T> & {
...
declare type Payload<S> = S | (() => AnyAsync<S>);
declare type UpdateResult<X extends AnyAsync<any>> = X extends (() => AnyAsync<infer R>) ? Future<R> : void;
export declare type UpdatableObject<S, F extends FindOrFilter, Q extends QueryStatus> = (Patch<S> & (F extends 'isFind' ? Replace<S> : {}) & ({
...
}) & (Readable<F extends 'isFilter' ? S[] : S>) & InvalidateCache & Remove);
export declare type UpdatableArray<S extends Array<any>, F extends FindOrFilter, Q extends QueryStatus> = (Q extends 'queried' ? (({
...
} & Remove) & (F extends 'isFind' ? Replace<S[0]> : {}) & (S[0] extends Array<any> ? {} : S[0] extends object ? UpdatableObject<S[0], F, Q> : UpdatablePrimitive<S[0], F, Q>)) : (({
...
} & RemoveAll & InsertMany<S> & InsertOne<S[0]> & ReplaceAll<S>) & (S[0] extends Array<any> ? {} : S[0] extends object ? UpsertMatching<S[0]> : {}) & (S[0] extends object ? (({
    [K in keyof S[0]]: (S[0][K] extends Array<any> ? UpdatableArray<S[0][K], 'isFilter', 'notQueried'> : S[0][K] extends object ? UpdatableObject<S[0][K], F, Q> : UpdatablePrimitive<S[0][K], F, Q>);
}) & PatchAll<S[0]>) : IncrementAll) & (Readable<F extends 'isFilter' ? S : S[0]>)));
export declare type UpdateOptions<H> = (H extends () => AnyAsync<any> ? CacheFor & OptimisticallyUpdateWith<H> : {}) | void;
export declare type UpdatablePrimitive<S, F extends FindOrFilter, Q extends QueryStatus> = ((Q extends 'notQueried' ? ReplaceAll<S> : F extends 'isFind' ? Replace<S> : {}) & (S extends number ? (Q extends 'notQueried' ? IncrementAll : Increment) : {}) & (Readable<F extends 'isFilter' ? S[] : S>) & InvalidateCache & Remove);
export interface UpsertMatching<S> {
    upsertMatching: {
        [K in keyof S]: S[K] extends object ? UpsertableObject<S, S> : UpsertablePrimitive<S>;
    };
}
...
export declare type UpdatableAny<T, F extends FindOrFilter, Q extends QueryStatus> = T extends Array<any> ? UpdatableArray<T, F, Q> : T extends object ? UpdatableObject<T, F, Q> : UpdatablePrimitive<T, F, Q>;
export declare type Comparators<T, S, F extends FindOrFilter> = {
...
export declare type Searchable<T, S, F extends FindOrFilter> = {
    [K in keyof S]: (S[K] extends object ? (Searchable<T, S[K], F> & Comparators<T, S[K], F>) : Comparators<T, S[K], F>);
};
...
export declare type Store<S> = Omit<S extends Array<any> ? UpdatableArray<S, 'isFilter', 'notQueried'> : S extends object ? UpdatableObject<S, 'isFind', 'queried'> : UpdatablePrimitive<S, 'isFind', 'queried'>, 'remove'>;
declare type DerivationCalculationInput<E> = E extends Readable<infer W> ? W : never;
export declare type DerivationCalculationInputs<T extends Array<Readable<any>>> = {
    [K in keyof T]: DerivationCalculationInput<T[K]>;
};
export interface Derivation<R> extends Read<R>, OnChange<R>, Invalidate {
}
export declare type FutureState<C> = {
    isLoading: boolean;
    wasRejected: boolean;
    wasResolved: boolean;
    error: any;
...
export declare type Augmentations = {
    selection: {
        [name: string]: <C>(selection: Readable<C>) => (...args: any[]) => any;
    };
    future: {
        [name: string]: <C>(future: Future<C>) => (...args: any[]) => any;
    };
    derivation: {
        [name: string]: <R>(derivation: Derivation<R>) => (...args: any[]) => any;
    };
    async: <C>(fnReturningFutureAugmentation: () => any) => Promise<C>;
};
export interface Async<C> {
}
export declare type AnyAsync<C> = Async<C> | Promise<C>;
export declare const Deferred: unique symbol;
export declare type OptionsForMakingAComponentStore = {
    /**
