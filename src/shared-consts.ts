export const errorMessages = {
  ...
  ILLEGAL_CHARACTERS_WITHIN_SELECTOR: (functionName: 'get' | 'getProp') => `Illegal characters detected in your ${functionName}() function. If you are attempting to find or filter array elements, then you can do so as follows: 'select(s => s.some.array).findWhere(s => s.id).eq(3)'`,
