...
import { defineDetachFromAppStore } from './operators-internal';
import {
...
export function createStoreCore<S, T extends Trackability>({
  ...
  const action = <C, X extends C & Array<any>>(selector: Selector<S, C, X>) => {
    ...
    const getCoreActionsState = () => ({
      updateState,
      selector,
      isNested: () => !!coreActions.detachFromAppStore,
      storeState,
      pathReader,
      storeResult,
      initialState,
    ...
    const coreActions = {
      ...
      defineDetachFromAppStore: defineDetachFromAppStore(() => currentState, updateState),
      defineReset: (
