...
test('should sort array by number ascending', () => {
  ...
  const sortedArr = store.arr.$createSortedList.$ascending();
  expect(sortedArr.$state).toEqual([1, 2, 3]);
  const sortedArr2 = store.arr.$createSortedList.$descending();
...
test('should read deep array element properties', () => {
  ...
  const sortedArr = store.arr.$createSortedList.$withId.id.$sortedBy.id.$descending();
...
test('should update correctly when element is pushed to ascending sort', () => {
  ...
  const store = createStore(state);
  const sortedArr = store.arr.$createSortedList.$withId.id.$sortedBy.id.$ascending();
  let changeCount = 0;
...
test('should sort by date correctly', () => {
  ...
  const sortedArr = store.arr.$createSortedList.$withId.id.$sortedBy.date.$ascending();
  expect(sortedArr.$state).toEqual([{ id: 2, date: new Date('2020-02-01') }, { id: 1, date: new Date('2020-06-01') }, { id: 3, date: new Date('2020-08-01') }]);
  const sortedArr2 = store.arr.$createSortedList.$withId.id.$sortedBy.date.$descending();
...
test('should sort array when entire array is replaced', () => {
  ...
  const store = createStore(state);
  const sort = store.arr.$createSortedList.$withId.id.$sortedBy.id.$ascending();
...
test('', () => {
  ...
  const m = store.arr.$createSortedList.$withId.id.$sortedBy.id.$ascending();
  // m.$onChange(() => {});
...
test('', () => {
  ...
  const s1 = store.notes.$createSortedList.$withId.id.$sortedBy.dateUpdated.$ascending();
