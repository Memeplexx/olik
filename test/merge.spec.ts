...
describe('merge', () => {
  ...
  it('should allow basic merges', () => {
    ...
    mergeStoreIfPossible({ store: select2, nameOfStoreToMergeInto: nameOfExistingStore });
  ...
  it('should not throw an error if no store exists to merge into', () => {
    ...
    mergeStoreIfPossible({ store: select, nameOfStoreToMergeInto: 'xxx' });
  })
  ...
  it('should throw an error if the existing stores state is a primitive', () => {
    ...
    expect(() => mergeStoreIfPossible({ store: selectNested, nameOfStoreToMergeInto: nameOfExistingStore }))
