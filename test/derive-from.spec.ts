...
describe('Memoize', () => {
  ...
  it('should deriveFrom() including a filter()', () => {
    ...
    let memoCalcCount = 0;
    const mem = deriveFrom(
      select(s => s.array).findWhere(e => e.id).isEq(2),
    ...
    mem.read();
    mem.read();
    select(s => s.array).findWhere(e => e.id).isEq(1).patch({ value: 'xxx' });
    expect(memoCalcCount).toEqual(1);
    select(s => s.array).findWhere(e => e.id).isEq(2).patch({ value: 'xxx' });
    mem.read();
