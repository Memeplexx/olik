...
describe('Fetcher', () => {
  ...
  it('should perform a basic fetch using replace() on a primitive including caching', done => {
    ...
    get().replace(cachedPromise(fetchArray).ttl(1000))
    setTimeout(() => {
      expect(get().read()).toEqual(1);
  ...
  //   }
  //   get().replaceAll(cachedPromise(fetchString).args(0, 10).ttl(1000))
  //     .then(res => get().replaceAll(cachedPromise(fetchString).args(0, 10).ttl(1000)))
  //     .then(res => expect(count).toEqual(1));
  // })
